...
def register_error_handlers(app: FastAPI) -> FastAPI:
    with tracer.start_as_current_span("_register_error_handlers"):
        app.add_exception_handler(ValidationBusinessError, http_validation_business_error_handler)
...
async def http_validation_business_error_handler(_: Request, exc: ValidationBusinessError) -> JSONResponse:
    with tracer.start_as_current_span("http_business_error_handler"):
        logger.info(exc)
        return ORJSONResponse(status_code=408, content=_as_camelized_dict(exc))
