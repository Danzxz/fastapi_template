...
class HealthCheckCommander:
    ...
    async def db_check_route(self) -> WorkingCapacityResult:
        """
        Эндпоинт для проверки соединения с БД

        :return: Результат проверки
        """
        with tracer.start_as_current_span("db_check_route"):
            return await self._form_working_capacity(
                self._db_handler, need_percentage=self._config.PERCENTAGE_MINIMUM_FOR_WORKING_CAPACITY
            )
    ...
    async def _form_working_capacity(
        ...
        with tracer.start_as_current_span("_form_working_capacity"):
            ...
            async with TimeCatcher() as catcher:  # type: TimeCatcher
                for name, command in command_handler.commands.items():  # type: str, BaseCommand
